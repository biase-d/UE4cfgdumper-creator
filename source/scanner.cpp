...
template <typename T> T searchString(char* buffer, T string, u64 buffer_size, bool null_terminated, bool whole);
...
void initialize_scanner() {
    ...
    utf_encoding = 0;
    mappings_count = 0;
    if (memoryInfoBuffers) {
        delete[] memoryInfoBuffers;
        memoryInfoBuffers = nullptr;
    }
}
...
bool run_scan() {
    ...
    if (!hasCheatProcess) dmntchtForceOpenCheatProcess();

    if (R_FAILED(dmntchtGetCheatProcessMetadata(&cheatMetadata))) {
        printf("Failed to get cheat process metadata.\n");
        deinitialize_scanner();
        return false;
    }
    
    if (R_FAILED(dmntchtGetCheatProcessMappingCount(&mappings_count))) {
        printf("Failed to get mapping count.\n");
        deinitialize_scanner();
        return false;
    }
...
template <typename T> T searchString(char* buffer, T string, u64 buffer_size, bool null_terminated, bool whole) {
	...
	if (whole) {
		while ((uint64_t)result != (uint64_t)&buffer[buffer_size]) {
			if (result > buffer && !result[-1 * sizeof(std::remove_pointer_t<std::remove_reference_t<T>>)])
				return (T)result;
			result = std::search(result + 1, buffer_end, (char*)string, (char*)string_end);
		}
	}
...
bool checkIfUE4game() {
	...
	while (i < mappings_count) {
		if ((memoryInfoBuffers[i].perm & Perm_R) == Perm_R && (memoryInfoBuffers[i].perm & Perm_Rx) != Perm_Rx && memoryInfoBuffers[i].type == MemType_CodeStatic) {
			...
			char* result = searchString(buffer_c, (char*)test_4, memoryInfoBuffers[i].size, false, false);
			if (result) {
				ue4_sdk = result;
				delete[] buffer_c;
				return true;
			}
			result = searchString(buffer_c, (char*)test_5, memoryInfoBuffers[i].size, false, false);
			if (result) {
				ue4_sdk = result;
				isUE5 = true;
				delete[] buffer_c;
...
uint8_t testRUN() {
	...
	while (i < mappings_count) {
		if ((memoryInfoBuffers[i].perm & Perm_Rw) == Perm_Rw && memoryInfoBuffers[i].type == MemType_Heap) {
			...
			char* buffer_c = new char[memoryInfoBuffers[i].size];
			dmntchtReadCheatProcessMemory(memoryInfoBuffers[i].addr, (void*)buffer_c, memoryInfoBuffers[i].size);
            char* result = searchString(buffer_c, (char*)UE4settingsArray[0].description, memoryInfoBuffers[i].size, false, false);
			if (result) {
			...
			if (!encoding) {
				char16_t* result16 = searchString(buffer_c, utf16_string, memoryInfoBuffers[i].size, false, false);
				if (result16) {
			...
			if (!encoding) {
				char32_t* result32 = searchString(buffer_c, utf32_string, memoryInfoBuffers[i].size, false, false);
				if (result32) {
...
char* findStringInBuffer(char* buffer_c, size_t buffer_size, const char* description) {
	char* result = nullptr;
	if (utf_encoding == 8) {
		result = (char*)searchString(buffer_c, (char*)description, buffer_size, false, false);
	}
		...
		result = (char*)searchString(buffer_c, utf16_string, buffer_size, false, false);
		delete[] utf16_string;
	}
	else {
		...
		result = (char*)searchString(buffer_c, utf32_string, buffer_size, false, false);
		delete[] utf32_string;
	}
	return result;
}
...
void SearchFramerate() {
	...
	for (size_t i = 0; i < mappings_count; i++) {
		...
		if (CTS_result) {
			ptrdiff_t CTS_diff = (uint64_t)CTS_result - address;
		...
		for (size_t x = 0; x < mappings_count; x++) {
			if ((memoryInfoBuffers[x].perm & Perm_Rx) != Perm_Rx && (memoryInfoBuffers[x].type == MemType_CodeMutable || memoryInfoBuffers[x].type == MemType_CodeWritable)) {
				if (memoryInfoBuffers[x].addr < cheatMetadata.main_nso_extents.base) continue;
				if (memoryInfoBuffers[x].addr >= cheatMetadata.main_nso_extents.base + cheatMetadata.main_nso_extents.size) continue;
				if (memoryInfoBuffers[x].size > 200'000'000) continue;
	...
	for (size_t y = 0; y < mappings_count; y++) {
		...
		if (it == &buffer_two[memoryInfoBuffers[y].size]) {
			it = std::search(buffer_two, &buffer_two[memoryInfoBuffers[y].size], pattern_3, &pattern_3[sizeof(pattern_3)]);
			pattern_number = 3;
		}
		if (it == &buffer_two[memoryInfoBuffers[y].size]) {
		...
		if (it != &buffer_two[memoryInfoBuffers[y].size]) {
			...
			if (insn && insn -> instr_id == AD_INSTR_ADRP) {
				...
				} else if (insn) { printf("Second instruction is not expected LDR or STR! %s\n", insn -> decoded); ArmadilloDone(&insn); }
			} else if (insn) { printf("First instruction is not ADRP! %s\n", insn -> decoded); ArmadilloDone(&insn); }
...
void searchDescriptionsInRAM() {
	...
	for (size_t x = 0; x < UE4settingsArray.size(); x++) {
		if (!UE4checkedList[x]) {
			if (isUE5) {
				if (UE5DeprecatedUE4Settings.count(UE4settingsArray[x].commandName)) {
					continue;
				}
			}
			if (UE4alternativeDescriptions1.count(UE4settingsArray[x].commandName)) {
				i = 0;
				while (i < mappings_count) {
					if ((memoryInfoBuffers[i].perm & Perm_Rw) == Perm_Rw && memoryInfoBuffers[i].type == MemType_Heap) {
						if (memoryInfoBuffers[i].size > 200'000'000) {
						...
						dmntchtReadCheatProcessMemory(memoryInfoBuffers[i].addr, (void*)buffer_c, memoryInfoBuffers[i].size);
						char* result = nullptr;
						result = findStringInBuffer(buffer_c, memoryInfoBuffers[i].size, UE4alternativeDescriptions1.at(UE4settingsArray[x].commandName).c_str());
						if (result) {
							ptrdiff_t diff = (uint64_t)result - (uint64_t)buffer_c;
							uint64_t string_address = memoryInfoBuffers[i].addr + diff;
		...
		if (isUE5 && !UE4checkedList[x]) {
			if (UE4toUE5alternativeDescriptions1.count(UE4settingsArray[x].commandName)) {
				i = 0;
				while (i < mappings_count) {
					if ((memoryInfoBuffers[i].perm & Perm_Rw) == Perm_Rw && memoryInfoBuffers[i].type == MemType_Heap) {
						if (memoryInfoBuffers[i].size > 200'000'000) {
						...
						dmntchtReadCheatProcessMemory(memoryInfoBuffers[i].addr, (void*)buffer_c, memoryInfoBuffers[i].size);
						char* result = nullptr;
						result = findStringInBuffer(buffer_c, memoryInfoBuffers[i].size, UE4toUE5alternativeDescriptions1.at(UE4settingsArray[x].commandName).c_str());
						if (result) {
							ptrdiff_t diff = (uint64_t)result - (uint64_t)buffer_c;
							uint64_t string_address = memoryInfoBuffers[i].addr + diff;
							if (searchPointerInMappings(string_address, UE4settingsArray[x].commandName, UE4settingsArray[x].type, x)) {
		...
		if (isUE5 && !UE4checkedList[x]) {
			if (UE4toUE5alternativeDescriptions2.count(UE4settingsArray[x].commandName)) {
				i = 0;
				while (i < mappings_count) {
					if ((memoryInfoBuffers[i].perm & Perm_Rw) == Perm_Rw && memoryInfoBuffers[i].type == MemType_Heap) {
						if (memoryInfoBuffers[i].size > 200'000'000) {
						...
						dmntchtReadCheatProcessMemory(memoryInfoBuffers[i].addr, (void*)buffer_c, memoryInfoBuffers[i].size);
						char* result = nullptr;
						result = findStringInBuffer(buffer_c, memoryInfoBuffers[i].size, UE4toUE5alternativeDescriptions2.at(UE4settingsArray[x].commandName).c_str());
						if (result) {
							ptrdiff_t diff = (uint64_t)result - (uint64_t)buffer_c;
							uint64_t string_address = memoryInfoBuffers[i].addr + diff;
							if (searchPointerInMappings(string_address, UE4settingsArray[x].commandName, UE4settingsArray[x].type, x)) {
	...
	for (size_t x = 0; x < UE4settingsArray.size(); x++) {
		if (UE4checkedList[x]) continue;
		if (UE4alternativeDescriptions1.count(UE4settingsArray[x].commandName) && !UE4toUE5alternativeDescriptions1.count(UE4settingsArray[x].commandName) && !UE4toUE5alternativeDescriptions2.count(UE4settingsArray[x].commandName)) {
		...
		if (isUE5 && UE4toUE5alternativeDescriptions1.count(UE4settingsArray[x].commandName) && !UE4toUE5alternativeDescriptions2.count(UE4settingsArray[x].commandName)) {
			printf(CONSOLE_RED "!" CONSOLE_RESET ": " CONSOLE_CYAN "%s" CONSOLE_RESET " was not found even with UE5 alt 1 description!\n", UE4settingsArray[x].commandName);
			consoleUpdate(NULL);
			continue;
		}
		if (isUE5 && UE4toUE5alternativeDescriptions2.count(UE4settingsArray[x].commandName)) {
			printf(CONSOLE_RED "!" CONSOLE_RESET ": " CONSOLE_CYAN "%s" CONSOLE_RESET " was not found even with UE5 alt 2 description!\n", UE4settingsArray[x].commandName);
			consoleUpdate(NULL);
			continue;
		}
		if (isUE5 && UE5DeprecatedUE4Settings.count(UE4settingsArray[x].commandName)) {
			printf(CONSOLE_RED "!" CONSOLE_RESET ": " CONSOLE_CYAN "%s" CONSOLE_RESET " was deprecated on UE5!", UE4settingsArray[x].commandName);
			if (UE5DeprecatedUE4Settings.at(UE4settingsArray[x].commandName).compare("")) {
				printf(CONSOLE_CYAN " %s" CONSOLE_RESET, UE5DeprecatedUE4Settings.at(UE4settingsArray[x].commandName).c_str());
			}
			printf("\n");
			consoleUpdate(NULL);
			continue;
...
void searchDescriptionsInRAM_UE5() {
	...
	for (size_t x = 0; x < UE5settingsArray.size(); x++) {
        if(UE5checkedList[x]) continue;
		if (UE5alternativeDescriptions1.count(UE5settingsArray[x].commandName)) {
			i = 0;
			while (i < mappings_count) {
				if ((memoryInfoBuffers[i].perm & Perm_Rw) == Perm_Rw && memoryInfoBuffers[i].type == MemType_Heap) {
					if (memoryInfoBuffers[i].size > 200'000'000) {
					...
					dmntchtReadCheatProcessMemory(memoryInfoBuffers[i].addr, (void*)buffer_c, memoryInfoBuffers[i].size);
					char* result = nullptr;
					result = findStringInBuffer(buffer_c, memoryInfoBuffers[i].size, UE5alternativeDescriptions1.at(UE5settingsArray[x].commandName).c_str());
					if (result) {
						ptrdiff_t diff = (uint64_t)result - (uint64_t)buffer_c;
						uint64_t string_address = memoryInfoBuffers[i].addr + diff;
